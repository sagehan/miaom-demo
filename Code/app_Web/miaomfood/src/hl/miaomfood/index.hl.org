...
(js/setInterval rpc/fetch-raw-db 20000)
;; cell formula ============================================================
;;(defc= datoms rpc/raw-db)
(defc= datoms rpc/raw-db)
(defc schema {:website/notices   {:db/cardinality :db.cardinality/many
               ...
               :cuisine/species   {:db/cardinality :db.cardinality/many
                                   :db/valueType :db.type/ref }})
(defc=  conn  (d/create-conn schema))
(d/transact! @conn  (vec @datoms))

(defc= site-title (d/q '[:find ?t . :where [_ :website/title ?t]] @conn))
